...
class myView(object):
    ...
    def update_view(self):
    	...
    	outfile = ("/home/brian/%s" % now.strftime("SPI_%Y%m%d%H%M%S") + ".jpg")
...
#	 comtest=subprocess.Popen('', bufsize=0, executable=none, stdin=None, stdout=None, stderr=None, preexec_fn=None, close_fds=False, cwd=None, env=None, universal_newlines=False, startupinfo=None, creationflags=0)
	...
	subprocess.check_call(cmdlostart,shell=True)
 	return []
...
#       time.sleep(2)
	...
	cam.start()
	img = cam.get_image()
	pygame.image.save(img,"/home/brian/capture.jpg")
#	time.sleep(2)
	cam.stop()
#	time.sleep(2)
#	subprocess.check_call(cmdlostart,shell=True)
...
#	imp += ["-dissolve", "25"]
#	imp += ["-gravity", "South"]
#	subprocess.check_call(["composite -dissolve 25"],["-gravity"],["South"],[tfile],[outfile],[outfile])
	return []
